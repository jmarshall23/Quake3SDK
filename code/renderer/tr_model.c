...
qhandle_t RE_RegisterModel( const char *name ) {
	...
	for ( lod = MD3_MAX_LODS - 1 ; lod >= 0 ; lod-- ) {
		...
		if ( ident == MDR_IDENT ) {
			loaded = R_LoadMDR(mod, buf, len, name);
		} else {
			if ( ident != MD3_IDENT ) {
...
static qboolean R_LoadMDR(model_t* mod, void* buffer, int filesize, const char* mod_name)
{
	...
	mod->dataSize += size;
	mod->mdr = mdr = ri.Hunk_Alloc(size, h_low);
...
int R_LerpTag( orientation_t *tag, qhandle_t handle, int startFrame, int endFrame, 
	...
	if ( !model->md3[0] ) {
		...
		{
			start = R_GetAnimTag((mdrHeader_t*)model->mdr, startFrame, tagName, &start_space);
			end = R_GetAnimTag((mdrHeader_t*)model->mdr, endFrame, tagName, &end_space);
		}
		else
		{
			AxisClear(tag->axis);
